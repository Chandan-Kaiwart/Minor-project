...
// ====== MAIN TAB: PS1 (AI Architecture Pre-check merged into backend evaluator) ======
...
async function encryptGitHubProject() {
    ...
    btn.innerText = "Encrypting & Sealing...";
    btn.disabled = true;
...
async function simulatePS1() {
    ...
    resDiv.innerHTML = '<span style="color:var(--warning);"><span class="type-cursor" style="display:inline-block; height:10px;"></span> Running AI Architecture scan + NIST Evaluation Workbench...</span>';

    const input = txt.toLowerCase();
    let aiNote = '';

    // === AI Architecture Pre-check ===
    if (input.includes('cnn') || input.includes('conv2d')) {
        aiNote = `<div style="padding:10px; border-left:3px solid var(--primary); background:rgba(0,250,136,0.05); margin-top:10px; font-size:0.78rem;">
            <b style="color:var(--primary);">[AI ARCH] CNN Visual Feature Extractor detected.</b><br>
            Recommended: <b>ML-KEM-1024</b> for edge-vision stream shielding (FIPS-203 Level 5).
        </div>`;
    } else if (input.includes('lstm') || input.includes('rnn')) {
        aiNote = `<div style="padding:10px; border-left:3px solid var(--primary); background:rgba(0,250,136,0.05); margin-top:10px; font-size:0.78rem;">
            <b style="color:var(--primary);">[AI ARCH] LSTM Time-Series Engine detected.</b><br>
            Recommended: <b>ML-KEM-768</b> for high-velocity sequence shielding.
        </div>`;
    } else if (input.includes('rf') || input.includes('randomforest') || input.includes('ensemble') || input.includes('classifier')) {
        aiNote = `<div style="padding:10px; border-left:3px solid var(--primary); background:rgba(0,250,136,0.05); margin-top:10px; font-size:0.78rem;">
            <b style="color:var(--primary);">[AI ARCH] Random Forest Ensemble detected.</b><br>
            Recommended: <b>ML-DSA-65</b> for decision-node telemetry signing.
        </div>`;
    } else if (input.includes('model') || input.includes('train') || input.includes('neural')) {
        aiNote = `<div style="padding:10px; border-left:3px solid var(--primary); background:rgba(0,250,136,0.05); margin-top:10px; font-size:0.78rem;">
            <b style="color:var(--primary);">[AI ARCH] Neural Network cluster detected.</b><br>
            Recommended: <b>ML-KEM-768</b> standard PQC shielding for model weights.
        </div>`;
    }
    ...
    setTimeout(async () => {
        try {
            ...
            const data = await res.json();
            let color = data.score > 80 ? 'var(--primary)' : (data.score > 40 ? 'var(--warning)' : 'var(--danger)');
            let colorBg = data.score > 80 ? 'rgba(0,250,136,0.1)' : (data.score > 40 ? 'rgba(219,147,34,0.1)' : 'rgba(248,75,75,0.1)');
            ...
            resDiv.innerHTML = `
                <div style="margin-top:10px; padding:15px; border:1px dashed ${color}; border-radius:8px; background:${colorBg}; display:flex; flex-direction:column; gap:8px;">
                    <div style="color:${color}; font-weight:800; font-size:1.1rem;">NIST Security Score: ${data.score}/100</div>
...
const matrixChars = '01アイウエオ量子暗号ℤℚℝ∀∃∑∏←→↑↓⊕⊗';
const fontSize = 13;
let columns = canvas ? Math.floor(canvas.width / fontSize) : 80;
    ...
    try:
        ...
        if method == 'branch':
            ...
            try:
                ...
                if 'sha' not in tree_resp:
                    return jsonify({'status': 'error', 'message': f"GitHub API Tree Create Error: {tree_resp.get('message', str(tree_resp))}"})
                ...
                if 'sha' not in commit_resp:
                    return jsonify({'status': 'error', 'message': f"GitHub API Commit Error: {commit_resp.get('message', str(commit_resp))}"})
                new_commit_sha = commit_resp['sha']
                
                branch_name = f"pqc-hardened-{int(time.time())}"
                ref_resp = requests.post(f"https://api.github.com/repos/{owner}/{repo_str}/git/refs", json={'ref': f'refs/heads/{branch_name}', 'sha': new_commit_sha}, headers=headers_api).json()
                if 'ref' not in ref_resp:
                    return jsonify({'status': 'error', 'message': f"GitHub API Push Error: {ref_resp.get('message', str(ref_resp))}"})
                
                return jsonify({
                    'status': 'success',
                    'branch': branch_name,
                    'message': f'Successfully pushed {len(tree_items)} encrypted files to GitHub branch {branch_name}.'
                })
